...
export const GoogleSignInButton = ({ onError, label = 'Continue with Google' }) => {
  ...
  const hiddenGisRef = useRef(null);

  useEffect(() => {
    // Fetch Google Client ID from backend as source of truth
    authAPI.getGoogleConfig()
      ...
      .catch((err) => {
        console.warn('Could not fetch Google config from backend, using environment fallback:', err);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    const initGoogleGIS = () => {
      ...
      try {
        ...
        // Render hidden GIS button in container to allow seamless click forwarding
  ...
  const handleClick = () => {
    const activeClientId = clientId || import.meta.env.VITE_GOOGLE_CLIENT_ID;
    if (!activeClientId) {
      if (onError) onError('● Unable to connect to the server. Check that the backend is running.');
      return;
    }
    ...
    if (window.google?.accounts?.id) {
      ...
      // Also simulate click on rendered GIS iframe if prompt didn't show immediately
      const iframeBtn = hiddenGisRef.current?.querySelector('div[role="button"]');
      if (iframeBtn) {
        iframeBtn.click();
      }
    }
  ...
  return (
    <div className="google-oauth-wrapper" style={{ position: 'relative', width: '100%' }}>
      {/* Official custom styled button according to design requirements */}
      <button
        type="button"
        onClick={handleClick}
        disabled={loading}
      ...
      {/* Hidden GIS iframe for browser click capture & credential response */}
      <div
        ref={hiddenGisRef}
        className="google-gis-hidden-overlay"
        aria-hidden="true"
      />
    </div>
  );
