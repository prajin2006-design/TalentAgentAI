...
- **AI Integration**: OpenAI API server-side through `POST /api/ai/chat`, with bounded persisted conversations and profile-aware Markdown responses. Configure `OPENAI_API_KEY` and `OPENAI_MODEL` in `backend/.env`; the key never reaches React.
